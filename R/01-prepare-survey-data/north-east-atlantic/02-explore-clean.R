# Purpose:   Filter and clean of raw DATRAS output from
#            01-download-datras.R. 
#
# Input:     data/clean/clean_survey_data.rds
#
# Output:    data/clean/ca_clean.rds
#            data/clean/hl_clean.rds
#            output/check_species_laa/*.png
#            output/laa_data_overview.png
#            output/data_format_template.xlsx
#
# Modified:  2026-06-22
# Author:    Max Lindmark
...
results <- readRDS(file.path(home, "R/01-prepare-survey-data/north-east-atlantic/output/merged_survey_data.rds"))
...
ca_all <- bind_rows(lapply(names(results), function(s) {
  ...
  df |> mutate(survey = s, lngt_code = as.character(lngt_code),
               year = year(date))
}))
...
ca_all <- ca_all |> tidylog::filter(!is.na(age), age >= 0)

# Lon/lat sanity (Atlantic + European shelf)
#ca_all <- ca_all |> tidylog::filter(lon > -40, lon < 40, lat > 30, lat < 90)

# Keep only species x survey combinations with >= 10 years each having >= 10 obs
eligible <- ca_all |>
  summarise(n = n(), .by = c(survey, species, year)) |>
  filter(n >= 10) |>
  summarise(n_years = n_distinct(year), .by = c(survey, species)) |>
  filter(n_years >= 10)

ca_all <- ca_all |>
  tidylog::semi_join(eligible, by = c("survey", "species"))
...
dir.create(file.path(home, "R/01-prepare-survey-data/datras/output/check_species_laa"), recursive = TRUE, showWarnings = FALSE)
...
for (sp in sort(unique(ca_all$species))) {
  ...
  fname <- file.path(
    home, "R/01-prepare-survey-data/north-east-atlantic/output/check_species_laa",
...
for (sp in problem_species) {
  ...
  fname <- file.path(
    home, "R/01-prepare-survey-data/datras/output/check_species_laa",
...
t <- ca_all |>
  summarise(n = n(), .by = c(species, year, survey)) |>
  mutate(tot_n = sum(n), .by = species) |> 
  ggplot(aes(as.numeric(as.character(year)), reorder(species, tot_n), size = n, color = survey)) +
  geom_point(alpha = 0.7) +
  scale_x_continuous(breaks = seq(1950, 2030, by = 4)) +
  scale_color_viridis_d() +
  scale_size_area(max_size = 8) +
  labs(x = "Year", y = NULL, size = "N individuals", color = "Survey")

ggsave(paste0(home, "/R/01-prepare-survey-data/datras/output/species_overview.png"), width = 25, height = 25, unit = "cm")
...
hl_all <- hl_all |> mutate(region = "North East Atlantic (DATRAS)")

saveRDS(ca_all, file.path(home, "data/clean/ca_clean.rds"))
saveRDS(hl_all, file.path(home, "data/clean/hl_clean.rds"))
...
saveRDS(ca_all, file.path(home, "data/clean/north-east-atlantic/length_clean.rds"))
saveRDS(hl_all, file.path(home, "data/clean/north-east-atlantic/catch_clean.rds"))
